...
struct SinglePokemonInfo {
    ...
    init(id: String, name: String, baseExperience: String, height: String, weight: String,
         abilities: [PokemonAbilities], forms: [PokemonNameUrl], gameIndices: [PokemonGameIndices],
         heldItems: [PokemonHeldItems], locationAreaEncounters: String, moves: [PokemonMoves],
         species: PokemonNameUrl, sprites: PokemonSprites, stats: [PokemonStats],
         types: [PokemonTypes], pastTypes: [PokemonPastTypes]) {
        self.id = id
    ...
    init(response: PokemonListInfoDTO) {
        ...
        self.gameIndices = response.gameIndices.map { PokemonGameIndices(gameIndex: String($0.gameIndex),
                                                                         version: PokemonNameUrl(name: $0.version.name,
                                                                                                 url: $0.version.url)) }
        self.heldItems = response.heldItems.map { PokemonHeldItems(item: PokemonNameUrl(name: $0.item.name,
                                                                                        url: $0.item.url),
                                                                   versionDetails: $0.versionDetails.map {
            PokemonHeldItemVersionDetails(rarity: String($0.rarity),
                                          version: PokemonNameUrl(name: $0.version.name, url: $0.version.url)) }) }
        self.locationAreaEncounters = response.locationAreaEncounters
        self.moves = response.moves.map { PokemonMoves(move: PokemonNameUrl(name: $0.move.name, url: $0.move.url),
                                                       versionGroupDetails: $0.versionGroupDetails.map {
            PokemonMoveVersionGroupDetails(levelLearnedAt: String($0.levelLearnedAt),
                                           versionGroup: PokemonNameUrl(name: $0.versionGroup.name,
                                                                        url: $0.versionGroup.url),
                                           moveLearnMethod: PokemonNameUrl(name: $0.moveLearnMethod.name,
                                                                           url: $0.moveLearnMethod.url)) }) }
        ...
        self.stats = response.stats.map { PokemonStats(baseStat: String($0.baseStat),
                                                       effort: String($0.effort),
                                                       stat: PokemonNameUrl(name: $0.stat.name,
                                                                            url: $0.stat.url)) }
        self.types = response.types.map { PokemonTypes(slot: String($0.slot),
                                                       type: PokemonNameUrl(name: $0.type.name,
                                                                            url: $0.type.url)) }
        self.pastTypes = response.pastTypes.map { PokemonPastTypes(generation: PokemonNameUrl(name: $0.generation.name,
                                                                                              url: $0.generation.url),
                                                                   types: $0.types.map {
            PokemonTypes(slot: String($0.slot),
                         type: PokemonNameUrl(name: $0.type.name,
                                              url: $0.type.url)) }) }
    }
...
extension PokemonMoveVersionGroupDetails: Hashable {
    ...
    static func == (lhs: PokemonMoveVersionGroupDetails, rhs: PokemonMoveVersionGroupDetails) -> Bool {
        ...
        lhs.versionGroup == rhs.versionGroup &&
        lhs.moveLearnMethod == rhs.moveLearnMethod
    }
}
