...
class PokemonListDomainMapper {
    func map(domain: PokemonListResponseDTO) -> PokemonListResponse {
        ...
        for entry in domain.results {
            pokemonEntry.append(PokemonEntry(id: extractID(from: entry.url), name: entry.name, url: URL(string: entry.url)))
        }
    ...
    func map(infoDTO: PokemonListInfoDTO) -> PokemonListInfo {
        return PokemonListInfo(id: String(infoDTO.id), name: infoDTO.name, image: URL(string: infoDTO.sprites.frontDefault))
    }
}
