...
final class SinglePokemonRepositoryTests: XCTestCase {
    ...
    func test_getPokemonInfo_return_success() async throws {
        // GIVEN
        let apiDataSource = APISinglePokemonDataSourceStub(pokemonList:
...
private extension SinglePokemonInfo {
    static func makeSinglePokemonInfo() -> SinglePokemonInfo {
        return SinglePokemonInfo(id: "1", name: "test name 1", baseExperience: "64", height: "100", weight: "2",
                                 abilities: [], forms: [], gameIndices: [], heldItems: [],
                                 locationAreaEncounters: "test pokemonListInfoDTO locationareancounters",
                                 moves: [], species: PokemonNameUrl(name: "test species name", url: "test species url"),
                                 sprites: PokemonSprites(frontDefault: "test pokemonListInfoDTO frontdefault",
                                                         backDefault: "", backShiny: "", frontShiny: ""),
                                 stats: [], types: [], pastTypes: [])
    }
}
